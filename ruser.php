...
function gettoken($url)
{
    ...
    $result =  mysql_query("SELECT * FROM reader where openid='$user_open'");
        $num=mysql_num_rows($result);

        if($num == '0'){

            $result2 =  mysql_query("SELECT * FROM reader");
            $idnum = mysql_num_rows($result2);
            $id = $idnum++ ;
            $sql="INSERT INTO reader(openid,headimg,nickname,id) VALUES ('$user_open','$user_headimg','$user_nick','$id')";
